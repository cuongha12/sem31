...
const Size = () => {
    const { size, axiosJWT, user, getSize } = useContext(AppContext)
        ...
        },
        {
            ...
            render: (e) => (
                <Switch defaultChecked={e.status} onChange={async (vale) => {
                    try {
                        const res = await axiosJWT.put(`/size/update/${e?.id}`, {
                            status: vale
                        }, {
                            headers: {
            ...
            render: (e) => (
                <Space>
                    ...
                    <Button icon={<DeleteOutlined />} danger size={'middle'} type="primary" onClick={async () => {
                        try {
                            const res = await axiosJWT.delete(`/size/delete/${e?.id}`, {
                                headers: {
                                    Authorization: `Bearer ${user?.accessToken}`
                                }
                            })
    ...
    return (
        <div>
            <div>
                ...
                <Table scroll={{
                    x: 1300,
                }} rowSelection={rowSelection} columns={columns} dataSource={size} />
                {
                    open && <CreateSize open={open} setOpen={setOpen} record={record} title={title} />
                }
            </div>
        </div>
    )
