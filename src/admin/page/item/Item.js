...
const Item = () => {
    ...
    const { item, axiosJWT, user, getItem } = useContext(AppContext)
    // const start = () => {
    //     setLoading(true);
        ...
        },
        {
            ...
            render: (e) => (
                <Switch defaultChecked={e.status} onChange={async (vale) => {
                    try {
                        const res = await axiosJWT.put(`/item/update/${e?.id}`, {
                            status: vale
                        }, {
                            headers: {
            ...
            render: (e) => (
                <Space>
                    ...
                    <Button icon={<DeleteOutlined />} danger size={'middle'} type="primary" onClick={async () => {
                        try {
                            const res = await axiosJWT.delete(`/item/delete/${e?.id}`, {
                                headers: {
                                    Authorization: `Bearer ${user?.accessToken}`
                                }
                            })
    ...
    return (
        <div>
            <div>
                ...
                <Table scroll={{
                    x: 1300,
                }} rowSelection={rowSelection} columns={columns} dataSource={item} />
                {
                    open && <CreateItem open={open} setOpen={setOpen} record={record} title={title} />
                }
            </div>
        </div>
    )
